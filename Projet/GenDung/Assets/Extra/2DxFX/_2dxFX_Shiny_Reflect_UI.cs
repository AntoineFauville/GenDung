...
[ExecuteInEditMode]
[AddComponentMenu("2DxFX/Standard/Shiny_Reflect_UI")]
[System.Serializable]
public class _2dxFX_Shiny_Reflect_UI : MonoBehaviour
{
    [HideInInspector]
    ...
    private string shader = "2DxFX/Standard/Shiny_Reflect_UI";
    [HideInInspector]
    [Range(0, 1)]
    ...
    void Update()
    {
        if ((ShaderChange == 0) && (ForceMaterial != null))
        {
            ...
            if (IsSprite)
            {
                SpriteRenderer.sharedMaterial = ForceMaterial;
            }
            else
            {
        ...
        if (IsSprite)
        {
            dfname = SpriteRenderer.sharedMaterial.shader.name;
        }
        else
        {
            if (CanvasImage.material == null) dfname = "Sprites/Default";
        }
        if (dfname == "Sprites/Default")
        {
            ...
            {
                if (CanvasImage.material == null) CanvasImage.material = ForceMaterial;
            }
            __MainTex2 = Resources.Load("_2dxFX_Gradient") as Texture2D;

            if (IsSprite)
            {
                SpriteRenderer.sharedMaterial.SetTexture("_MainTex2", __MainTex2);
            }
            else
            {
                if (CanvasImage.material == null) CanvasImage.material.SetTexture("_MainTex2", __MainTex2);
            }
        }
#endif
        if (ActiveChange)
        {
            if (IsSprite)
            {
                SpriteRenderer.sharedMaterial.SetFloat("_Alpha", 1 - _Alpha);
                if (UseShinyCurve)
                {
                    if (ShinyLightCurve != null) SpriteRenderer.sharedMaterial.SetFloat("_Distortion", ShinyLightCurve.Evaluate(ShinyLightCurveTime));
                    ShinyLightCurveTime += (Time.deltaTime / 8) * AnimationSpeedReduction;
                }
                else
                {
                    SpriteRenderer.sharedMaterial.SetFloat("_Distortion", Light);
                }

                SpriteRenderer.sharedMaterial.SetFloat("_Value2", LightSize);
                SpriteRenderer.sharedMaterial.SetFloat("_Value3", Intensity);
                SpriteRenderer.sharedMaterial.SetFloat("_Value4", OnlyLight);
                SpriteRenderer.sharedMaterial.SetFloat("_Value5", LightBump);
            }
            else
            {
    ...
    void OnDestroy()
    {
        ForceMaterial.SetFloat("_Distortion", 0f);
...
#if UNITY_EDITOR
[CustomEditor(typeof(_2dxFX_Shiny_Reflect_UI)), CanEditMultipleObjects]
public class _2dxFX_Shiny_Reflect_UI_Editor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        _2dxFX_Shiny_Reflect_UI _2dxScript = (_2dxFX_Shiny_Reflect_UI)target;
